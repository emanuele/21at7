...
def home_heater_temperature(T_home_current, T_external, T_heater_current, heating_schedule, T_heating, k_home_external, k_heater_on, k_heater_off, k_home_heater, t_step):
    ...
    T_home_next_external = T_external + (T_home_current - T_external) * np.exp(-k_home_external * t_step) # Newton's law of cooling
    
    if heating_schedule == 1: # if heating is turned on then the heater is heated:
        T_heater_next = T_heating + (T_heater_current - T_heating) * np.exp(-k_heater_on * t_step) # Newton's law of cooling
    else: # if heating is turned off then the heater is cooled according to home's temperature:
        T_heater_next = T_home_current + (T_heater_current - T_home_current) * np.exp(-k_heater_off * t_step) # Newton's law of cooling

    # In any case, home's temperature is dragged by heater's temperature
    T_home_next_heater = T_heater_current + (T_home_current - T_heater_current) * np.exp(-k_home_heater * t_step) # Newton's law of cooling
